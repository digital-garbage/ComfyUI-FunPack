...
**feedback_rating**: Answer to the queued feedback question on its own 1 to 6 scale. The `feedback_question` output includes the exact legend to use. Most questions tune presence, priority, balance, fidelity, stability, or preference. The refiner can also ask a category-discovery question for uncertain word groups: `1=general`, `2=concept`, `3=style`, `4=quality`, `5=character`, `6=details`.
...
**feedback_question**: Optional concept feedback prompt generated by the node when feedback mode is active. The node prioritizes category-discovery questions for unknown or auto-classified concept phrases, then uses later 1 to 6 answers to adjust per-concept weights.
