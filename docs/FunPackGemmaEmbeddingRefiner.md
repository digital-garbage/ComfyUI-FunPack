...
`im_feeling_lucky` is the automated prompt-composer path. Instead of requiring the current prompt to name every action, camera move, or styling cue, it learns which tokens and concepts the user tends to prefer and applies them directly to conditioning. Lucky can start a session by itself: the first Lucky run creates a discovery history entry and seeds prompt token/phrase memory, so later ratings can train Lucky without first running the classic refinement loop. When `clip` is connected, Lucky turns the learned sequence into text and sends it through `clip.tokenize(...)` plus `clip.encode_from_tokens_scheduled(...)`, then uses the encoded result as the canvas for refinement. If no encoder is connected but compatible stored conditioning exists, Lucky can use that learned canvas rather than the current prompt's weak or empty conditioning. If the prompt contains a known anchor, Lucky can add strongly rated missing neighbours that commonly work with that anchor. If the prompt is vague, it uses global learned preferences so the result still has something intentional to look at.

Lucky stores token, pair, context, prompt-order, and conditioning-canvas memory without a fixed Lucky cap. Tokens stay available unless their embeddings are invalid; bad ratings teach the refiner to avoid specific adjacent token pairs or contexts first. Missing-axis ratings are constructive: `Missing details`, `Missing concept`, `Missing quality`, and paired missing ratings mark prompt tokens as wanted but underrepresented, increasing per-axis missing pressure so required tokens reserve Lucky composition space even if their learned score is still low. With changing prompts, the previous generation's rating is applied to the previous prompt and its injected Lucky tokens even if Lucky expanded the output shape, then Lucky composes the current generation from that already-learned memory. The current prompt contributes fresh neutral tokens after current Lucky composition, and normal non-Lucky runs also seed reusable Lucky memory, so new words become material for future runs without needing Lucky to be active every time. It can run by itself or alongside `into_the_void`.

For performance, Lucky keeps storage uncapped but does not decode the whole memory every run. Canvas selection uses saved tensor metadata first and only decodes the chosen canvas. Token composition chooses from the full eligible bank, reserves room for current prompt anchors and repeated missing tokens, then decodes only the token vectors actually selected for the current generation. When `clip` is connected, the learned prompt is capped to a practical per-run concept count before re-encoding so the text encoder is not asked to process the whole memory. The CLIP/Gemma prompt path prefers learned comma/semicolon-separated concept phrases, falling back to loose tokens only when phrase memory is unavailable. It also tracks the rated position where each phrase tends to work best, then sorts combined Lucky phrases back toward those learned positions. If a phrase works in several positions, Lucky can choose between close placements with a rating-weighted random pick. Lucky runs also share one stable memory history inside the refinement session, so changing enhancer text does not force normal prompt-variant conditioning scans or create a new heavy prompt history on every run; status output reports that Lucky update count directly.
