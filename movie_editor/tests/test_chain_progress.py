...
def test_chain_progress_partial_read():
    begin("pid1", "run-a", 3)
    layout = [{"scene_index": 0, "start_frame": 0, "in_sec": 0.0}]
    update("run-a", 1, {"kind": "videos", "filename": "partial_s1.mp4", "subfolder": "funpack_chain_progress/run-a", "type": "temp"}, scene_layout=layout)
    ...
    assert snap["scene_layout"] == layout
