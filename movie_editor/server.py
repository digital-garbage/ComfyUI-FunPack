...
def _expanded_link_texts(target: Project, prompt: str) -> dict[str, str]:
    ...
    body, timed, weighted, blended = bridge.expand_prompt_with_windows(prompt, variables)
    ...
    return {
        ...
        # Timed phrases and phrase-blend spans stripped from `prompt`, as char spans on it.
        # `full_prompt` starts with `prompt`, so the same spans hold there.
        "timed": timed,
        "weighted": weighted,
        "blended": blended,
    }
...
if web is not None and PromptServer is not None:
    ...
    async def _detail_status(req):
        ...
        try:
            dp = _detail_module()
            return web.json_response({"key": key, "rows": dp.rows(key),
                                      "enabled": dp.collection_enabled()})
        except Exception as e:
            return web.json_response({"key": key, "rows": [], "enabled": False,
                                      "error": str(e)})

    @routes.post(UI_PREFIX + "/api/detail_probe")
    async def _detail_toggle(req):
        try:
            body = await req.json()
        except Exception:
            body = {}
        key = str(body.get("key") or "default").strip() or "default"
        dp = _detail_module()
        dp.set_collection_enabled(bool(body.get("enabled")))
        return web.json_response({"key": key, "rows": dp.rows(key),
                                  "enabled": dp.collection_enabled()})
