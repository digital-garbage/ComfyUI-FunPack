...
(function () {
  ...
  const SAMPLER_KNOBS = [
    ...
    { name: "h3_prompt_time", label: "Prompt is settled", kind: "float", default: 0.0, min: 0.0, max: 1.0, step: 0.01,
      ...
      detail: "The model tells every part of its sequence how finished it is, and it tells the prompt whatever it tells the picture — so early on, while the picture is still noise, the prompt is treated as unreliable too. This tells the prompt it is settled no matter where the picture is. Applies to your prompt only: a reference sits in the same sequence behind a \u0027Picture N\u0027 label, and that label is text too — an earlier build settled it as well, which made the model copy the reference image into the frame instead of following the prompt. Not learned from ratings: its off value sits at the end of its range rather than the middle, so the learner cannot explore around it safely." },
