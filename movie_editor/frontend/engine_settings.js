...
(function () {
  ...
  const SAMPLER_KNOBS = [
    ...
    { name: "anchor_shift", label: "Anchor shift (fake t2v)", kind: "bool", default: false,
      hint: "EXPERIMENTAL: keep the real, untouched i2v anchor — then delete it from the clip. The anchor is a pinned frame at position 0, so it holds character detail, style and composition better than anything that weakens it on the way in (ALG blurs it and loses detail; Best-FaceID tokens approximate it and lose some too), but it is also literally the first frame you see. This runs pass 1 down to the sigma below with the anchor pinned as normal, drops the first frames and slides everything left (clip length unchanged — no extra frames generated), then finishes the schedule with the pin gone. The result is an i2v generation that reads as t2v. Pass 2 always re-enters at the exact latent state for its first sigma, which is what keeps the shifted latent on-manifold. Skipped on continuation scenes and on scenes carrying guide frames or JoyAI audio memory, with the reason in the scene report. Validated live in continue mode." },
    { name: "anchor_shift_sigma", label: "Shift at sigma", kind: "float", default: 0.909, min: 0.0, max: 0.999, step: 0.001, dependsOn: "anchor_shift",
      hint: "The knob this feature exists to let you tune, in both modes. SET IT TO 0 for no second pass at all: the whole schedule runs exactly as if anchor shift were off (anchor pinned at full strength the whole way), and the head is then cut off the finished clip — zero extra sampling, and with 'Regrown tail starts from' = crop it is simply 'generate the video, then delete the opening'. At 0 the tail must be crop or wrap (nothing would denoise a refilled tail) and 'Regenerate audio in pass 2' is ignored. Above 0, pass 1 runs until the schedule first reaches this sigma, then the shift happens. 0.909 is where structure actually starts forming on the distilled schedule (1.0 and 0.975 are near-pure noise and carry almost no signal), so it's the earliest cut where the anchor has plausibly transferred anything — a starting guess, not a measured value. Higher = released sooner and cheaper, but the reference may never have landed. Lower = held longer, but more of pass 1's picture is thrown away by the slide and fewer steps remain to repair it." },
    { name: "anchor_shift_frames", label: "Frames to drop", kind: "int", default: 8, min: 8, max: 512, step: 8, dependsOn: "anchor_shift",
      hint: "How much of the front to drop at the shift, in real frames. 8 (one latent frame) removes just the anchor itself, which is usually not enough — the anchor is followed by a settling-in stretch where the shot is still leaving the reference still and little is happening, and on a prompt asking for immediate action that dead time is exactly what you want gone. 48 was the value that worked on a 768×768×305@30 chain with a quick-cut prompt; treat that as a starting point for this pipeline rather than a universal default. Raise it if the opening still looks like it departed from a photo. The clip doesn't get shorter — whatever is dropped here is regrown at the end by pass 2." },
    { name: "anchor_shift_restart_sigma", label: "Pass 2 restart sigma", kind: "float", default: 0.0, min: 0.0, max: 0.999, step: 0.001, dependsOn: "anchor_shift",
      hint: "Where pass 2 re-enters the schedule after the shift. Pass 1 always stops at the shift sigma either way — this only decides where the second half picks up. 0 (default) = CONTINUE: resume from exactly the state pass 1 handed over, nothing added, cost stays one full schedule. Above the shift sigma = REWIND: the shifted latent is re-noised from the cut back up to this sigma, then denoised down again — much more freedom to rebuild the regrown tail and blend the joint, at the cost of repeating those steps and partly overwriting what pass 1 established. The re-noise is exact for a half-denoised latent (it rebuilds the state at the higher sigma from what pass 1 produced plus a matched top-up of fresh noise), not the naive img2img re-noise that caused the under-denoising in the first live run. BELOW the shift sigma is refused: noise can be added back, never removed. Try a step or two above the cut if continue leaves a visible seam or a thin ending." },
    { name: "anchor_shift_tail", label: "Regrown tail starts from", kind: "combo", choices: ["extend_last", "wrap", "empty", "crop"], default: "extend_last", dependsOn: "anchor_shift",
    ...
    { name: "second_pass_restart_sigma", label: "Pass 2 re-enters at sigma", kind: "float", default: 0.0, min: 0.0, max: 0.999, step: 0.001, dependsOn: "second_pass",
      hint: "0 (default) = carry straight on from the cut: nothing is added and the total cost is unchanged. A value ABOVE the cut rewinds — the half-denoised latent is rebuilt at that higher sigma (exact for a mid-trajectory latent, not the naive img2img re-noise) and those steps run again, letting the model rework structure it already committed to, at the cost of repeating them. Below the cut is refused: noise can be added back, never removed." },
