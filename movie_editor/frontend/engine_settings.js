...
(function () {
  ...
  const SAMPLER_KNOBS = [
    ...
    { name: "h3_block_repeat_last_steps", label: "Only repeat the last N steps", kind: "int", default: 0, min: 0, max: 50, dependsOn: "h3_block_repeat",
      hint: "0 = repeat on every step (the original behaviour). Above 0, the repeat only fires on the final N denoise steps.",
      detail: "Every sweep result above was run with this at 0 — the repeat firing on EVERY step, including the earliest, near-pure-noise ones where the block is still deciding layout and motion, not detail. That is the likely reason repeat re-rolls the shot instead of sharpening it: doubling a block's say over what the shot even IS, before it exists yet, is a different operation than doubling its say over how a settled shot looks. Confining the repeat to the tail, after structure is already locked in by the earlier steps, is untested and may behave completely differently from every finding on this page — none of it carries over automatically. Counted in STEPS from the end of the schedule, not sigma, so the window lands on the same point of denoising regardless of step count or sampler." },
