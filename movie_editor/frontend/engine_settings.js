...
(function () {
  ...
  const SAMPLER_KNOBS = [
    ...
    { name: "anchor_shift_restart_sigma", label: "Pass 2 restart sigma", kind: "float", default: 0.0, min: 0.0, max: 0.999, step: 0.001, dependsOn: "anchor_shift",
      hint: "Where pass 2 re-enters. 0 (default) continues from where pass 1 stopped, so the total cost stays one full schedule. Setting it higher than the shift sigma rewinds — pass 2 re-noises the shifted latent back up and re-denoises from there, giving more freedom to rebuild the regrown tail and blend the joint, at the cost of repeating those steps and partly overwriting what pass 1 established. Try it if the seam is visible or the new ending looks thin." },
