...
(function () {
  ...
  const SAMPLER_KNOBS = [
    ...
    { name: "h3_causal_sink", label: "Chunks pinned from the start", kind: "int", default: 2, min: 1, max: 64,
      dependsOn: "h3_causal_chunks", dependsValue: true,
      hint: "Chunks that stay visible to every later chunk and are never forgotten. Chunk 0 is the prompt, so 2 means the prompt plus the opening shot — which is what holds a character together across a long clip. Higher costs memory." },
