...
(function () {
  ...
  const SAMPLER_KNOBS = [
    ...
    { name: "explore_first_step",    label: "Explore at the first step", kind: "bool", default: false,
      hint: "Tries a few different starting seeds for one throwaway step, scores them, and commits to whichever one the model liked best \u2014 before running the real generation from it. Needs 10+ rated generations before it does anything.",
      detail: "Branches into a few candidate seeds and runs each ONE throwaway denoising step (the point where different noise still means different overall motion), scores each with the same trained memory Output guidance uses, then runs the real, full generation from the single best-scoring seed. Cost is one extra step per candidate, not a second full generation. A candidate that fails is skipped, not fatal; if nothing is ready yet the original seed runs exactly as if this were off." },
    { name: "explore_first_step_candidates", label: "Candidate seeds", kind: "int", default: 3, min: 2, max: 8, step: 1, dependsOn: "explore_first_step",
    ...
    { name: "h3_explore_temperature", label: "Randomizer (H3, experimental)", kind: "float", default: 0.0, min: -0.9, max: 3.0, step: 0.1,
      hint: "Positive: flattens attention at the block(s) below so the model is less sure how to weigh what it's already conditioned on — variety that stays bounded by the prompt, unlike latent noise. Negative: the opposite — sharpens attention so it commits harder to what the prompt already put in context. 0 = off. Manual dial, NOT rating-driven or coupled to REINS." },
    { name: "h3_explore_temperature_block", label: "Randomizer block(s)", kind: "text", default: "40-49", dependsOn: "h3_explore_temperature",
      hint: "Same syntax as Repeat block(s): a single block, a range, or a comma list." },
    ...
    { name: "h3_repr_steering_passive_capture", label: "REINS: keep learning while off", kind: "bool", default: false,
      hint: "Banks REINS' liked-minus-disliked history at every candidate block even while REINS itself is off above — every rating still trains it, nothing is ever added back into the video. Turn this on to build up rating history before committing to steering for real, or to compare REINS off against REINS on without losing ratings made during the off half. No effect once REINS itself is on (that already captures unconditionally)." },
    { name: "h3_q_steer_block", label: "Query steering block(s) (H3, experimental)", kind: "text", default: "",
      hint: "EXPERIMENTAL, unvalidated, H3 only. A second REINS-style liked-minus-disliked direction — same math as REINS above, learned and applied on attention's QUERY vectors instead of a block's hidden state, from its own independent rating history (same ratings, separate store). Blank = off, nothing captured. A block, a range, or a comma list, same syntax as Repeat block(s); try 0,1 first, where REINS' own style effect turned out to live.",
      detail: "Steers WHICH keys the video rows attend to, rather than adding content to what flows downstream the way REINS does — a genuinely different lever, though whether it reads as visually different from REINS is still an open question worth A/B-ing. Cannot run at the same time as Video/audio attention decoupling below: that mechanism's masking defeats this one entirely, so it is not installed at all (and captures nothing) whenever that is also on — the console says so when it happens." },
    { name: "h3_q_steer_strength", label: "Query steering strength", kind: "float", default: 0.0, min: 0.0, max: 10.0, step: 0.01, dependsOn: "h3_q_steer_block",
      hint: "Fraction of each steered block's own query-row norm added along its learned direction. Needs 2+ liked and 2+ disliked ratings at that block before it actually steers — until then it only captures. Ceiling raised well past REINS' own 2.0 — this only reaches the output through softmax, which can be comparatively insensitive to a Q shift, so it may need a much bigger push than REINS' equivalent number to do the same amount of visible work." },
    ...
    { name: "h3_shadow_negative", label: "Shadow negative (H3, experimental)", kind: "bool", default: false,
      hint: "Gives the negative prompt a job at CFG 1: runs it through the same blocks as a second stream and pushes the real output away from it. Heavier than 'Use the negative prompt' below (a per-step attention pass instead of a one-time edit) — needs a negative prompt connected to do anything.",
      detail: "MiniMax H3 runs at CFG 1.0 always, so the negative branch is never evaluated and a plain negative prompt does nothing. This runs the negative text through the SAME DiT blocks as a second 'shadow' stream, block by block, and repels the real stream's attention output from it (NAG-style, norm-capped so it cannot run away). A different, pricier lever than negative_erase's one-time embedding projection — roughly 15-30% slower while it's on. By default it does NOT compose with REINS / Query steering / Video-audio decoupling / Repeat block(s) / Randomizer: whichever of those is enabled loses to this one, and the console says so — turn on 'Allow when REINS/Q-steer are active' below if you want both running. Unvalidated — ported from a community node, never judged by eye on this project's own output yet." },
    { name: "h3_shadow_negative_compose", label: "Allow when REINS/Q-steer are active", kind: "bool", default: false, dependsOn: "h3_shadow_negative",
      hint: "Off: shadow negative takes over every block, which also stops REINS/Query steering/Repeat block(s)/Randomizer/Video-audio decoupling from capturing or steering — ratings made while shadow negative is on won't train them. On: shadow negative leaves whichever block(s) those already use completely alone, so they keep learning from your ratings as normal, and only pushes on the rest. Not the same as literally running both on one block at once — they just stop overwriting each other. The give-up is for the WHOLE run, not just the steps the other mechanism actually uses — e.g. a Repeat block(s) 'last steps' window still reserves its block for shadow negative every step, not only the steps it repeats. Does NOT help while REINS is running AT ALL, steering or just capturing: it claims all 50 blocks the moment it's on, for a technical reason (an even allocation pattern comfy's memory manager requires), not because of the one block you picked for it to learn from — its strength only decides whether it actually injects, not whether it claims the block, so turning strength down to 0 frees nothing. Only turning REINS fully off gives this checkbox any blocks to work with, and that also stops REINS learning for that run." },
    { name: "h3_shadow_negative_video_scale", label: "Video push strength", kind: "float", default: 3.0, min: 1.0, max: 10.0, step: 0.1, dependsOn: "h3_shadow_negative",
      hint: "1 = off for video. Higher pushes the picture harder away from the negative." },
    { name: "h3_shadow_negative_audio_scale", label: "Audio push strength", kind: "float", default: 1.0, min: 1.0, max: 10.0, step: 0.1, dependsOn: "h3_shadow_negative",
      hint: "1 = off for audio (the default — most negative prompts describe unwanted VISUALS). Higher pushes the soundtrack away from the same negative text." },
    { name: "h3_shadow_negative_tau", label: "Guidance cap (tau)", kind: "float", default: 2.5, min: 1.0, max: 8.0, step: 0.05, dependsOn: "h3_shadow_negative",
      hint: "Caps how far the guided result's norm can grow relative to the un-guided one. Lower prevents overcooking; raise it only if the push strengths above feel capped too early." },
    { name: "h3_shadow_negative_alpha", label: "Blend (alpha)", kind: "float", default: 0.35, min: 0.0, max: 1.0, step: 0.05, dependsOn: "h3_shadow_negative",
      hint: "0 = ignore this entirely, 1 = fully guided. If the result looks overcooked, lower this before lowering tau." },
    { name: "h3_shadow_negative_start_percent", label: "Window start", kind: "float", default: 0.0, min: 0.0, max: 1.0, step: 0.01, dependsOn: "h3_shadow_negative",
      hint: "Where in the denoise schedule guidance starts (0 = the very first step)." },
    { name: "h3_shadow_negative_end_percent", label: "Window end", kind: "float", default: 0.60, min: 0.0, max: 1.0, step: 0.01, dependsOn: "h3_shadow_negative",
      hint: "Where guidance stops (0.60 = the first 60% of the schedule — the community default). Guidance late in a schedule tends to fight fine detail rather than composition." },
