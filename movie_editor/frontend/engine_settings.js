...
(function () {
  ...
  const SAMPLER_KNOBS = [
    ...
    { name: "h3_causal_chunks", label: "Remembering chunks (H3)", kind: "bool", default: false,
      hint: "Generates the clip in time chunks that each remember the ones before them, instead of denoising the whole clip at once — so a long clip stays one continuous shot instead of separate scenes stitched together.",
      detail: "Needs two things in Models: chunk_causal switched on for the diffusion model, and a chunk-causal LoRA (the RAVEN streaming one) added to the LoRA list. It is a LoRA on the ordinary H3 checkpoint, not a different model file, but it is not optional here: the chunked attention pattern is what it was trained to read. Without it this says so and the run continues normally. The wired sampler and CFG are not used on this path. There is no per-chunk redo: a chunk is committed the moment it finishes and the next is already built on it, so disliking one chunk means a new seed. EXPERIMENTAL." },
    { name: "h3_causal_step_rule", label: "Chunk step rule", kind: "combo", choices: ["consistency", "euler", "euler_ancestral"], default: "consistency",
      dependsOn: "h3_causal_chunks", dependsValue: true,
      hint: "How each chunk reaches its next noise level. 'consistency' is what the RAVEN LoRA was distilled for. 'euler' is the ordinary step used everywhere else, so a chunked run can be compared against a normal one without this being a second thing that changed." },
    { name: "h3_causal_sink", label: "Chunks pinned from the start", kind: "int", default: 2, min: 1, max: 64,
      dependsOn: "h3_causal_chunks", dependsValue: true,
      hint: "Chunks that stay visible to every later chunk and are never forgotten, counted from the front: 1 is the prompt, 2 adds the anchor image and any reference, 3 adds the opening shot. Higher costs memory.",
      detail: "The prompt is every moment of the clip, so it is always pinned. The anchor is not: in a dense render it is one pin among hundreds of frames, but pinned into a three-block context it is a third of everything the model can see — and the model then does what it was trained to do with a conditioning row, which is compose from it. If every chunk restarts from your input image instead of continuing the shot, drop this to 1." },
