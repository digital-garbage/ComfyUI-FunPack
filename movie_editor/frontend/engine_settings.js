...
(function () {
  ...
  const SAMPLER_KNOBS = [
    ...
    { name: "h3_causal_chunks", label: "Remembering chunks (H3)", kind: "bool", default: false,
      ...
      detail: "Needs the ComfyUI-MiniMax-H3-RAVEN-Streaming pack installed and its RAVEN LoRA loaded; the chunked attention pattern is what that LoRA was trained to read. Without either it says so and the run continues normally. The wired sampler and CFG are not used on this path. There is no per-chunk redo: a chunk is committed the moment it finishes and the next is already built on it, so disliking one chunk means a new seed. EXPERIMENTAL." },
