...
(function () {
  ...
  const SAMPLER_KNOBS = [
    ...
    { name: "h3_q_steer_strength", label: "Query steering strength", kind: "float", default: 0.0, min: 0.0, max: 1.0, step: 0.01, dependsOn: "h3_q_steer_block",
      hint: "Fraction of each steered block's own query-row norm added along its learned direction. Needs 2+ liked and 2+ disliked ratings at that block before it actually steers — until then it only captures." },
