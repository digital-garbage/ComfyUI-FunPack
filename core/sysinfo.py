"""Host facts for the About section -- the machine ComfyUI runs on, not the
browser's.

That distinction is the whole point on a rental: the frontend already knows
the laptop it is displayed on, and nobody cares. What matters is the box doing
the sampling -- which GPU, how much VRAM, which torch, how much disk is left
for outputs.

Everything here is best-effort and individually guarded: a missing probe
returns None and the panel renders a dash. Nothing in this module may raise,
because About is the one screen that should still work when the rest of the
install is broken.
"""
    ...
    try:
        if sys.platform == "darwin":
            ...
            if os.path.exists(cpuinfo_path):
                with open(cpuinfo_path, errors="replace") as f:
                    for line in f:
...
def _os_name():
    try:
        ...
        if os.path.exists(osr_path):
            with open(osr_path, errors="replace") as f:
                for line in f:
...
def _torch():
    ...
    try:
        ...
        pass
    # Which fast-attention backend is actually importable. This is the line that
    # answers "is sage actually installed on this rental?" without digging
    # through the launch args.
