// Thin backend client — FORKED from movie_editor/frontend/api.js on purpose:
// Easy Gen is served from /funpack/easy/ but there is no /funpack/easy/api/*
// backend (see easy_gen/server.py) — Easy Gen reuses the Movie Editor's
// project/generate/media/models API as-is, so BASE must stay pinned to
// /funpack/movie regardless of the page's own URL. Keep every other method
// here in sync with the Editor's api.js; only this constant differs.
(function () {
  const BASE = "/funpack/movie";
