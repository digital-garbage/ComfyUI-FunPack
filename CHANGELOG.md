...
Added **Karras-style stochastic churn** to the `FunPack Distilled Flow` sampler (`s_churn`, `s_tmin`, `s_tmax`) for variability and detail at zero extra model evaluations. Each step optionally re-noises the latent within `[s_tmin, s_tmax]` before the model eval, then denoises from the bumped sigma level — the model is still evaluated once per step, just at a slightly higher noise level. This keeps the distilled trajectory from freezing (more motion, less stale/locked imagery) and can sharpen by letting the model re-resolve. `sigma_hat` is clamped to the schedule start so the model never sees a higher-than-trained noise level, and the final jump-to-zero detail step is never churned. Default band `0.5`–`1.0` churns the structure/motion sigmas while protecting the sub-0.4 detail finish (per the empirical sigma findings). Churn supersedes the legacy `s_noise` ancestral path while active, and is video-stream-only on packed LTXAV latents. With `s_churn=0` the sampler is byte-identical to the previous deterministic ODE. Wired into Studio's **Distilled Flow** sampler config panel.
