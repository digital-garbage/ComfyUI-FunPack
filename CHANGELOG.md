...
**Latent normalization stacks on the Hybrid and Distilled samplers too.** The same video-only anti-overbake normalization from the Normalizing sampler is now an opt-in `normalize_strength` / `normalize_start_sigma` on the Hybrid Euler 2S (CONST/RF path) and Distilled Flow samplers, so you can keep their features (AB2, Heun, velocity bias, rescue) *and* get the latent-health correction. Default off (no behaviour change); audio is never touched. Exposed on both nodes and in their Studio panels.

**FunPack Normalizing Sampler** — a new `SAMPLER` (selectable in Studio alongside Hybrid Euler 2S / Distilled Flow / KSampler) built for distilled few-step LTXAV at CFG=1, where the model is already extracted and there's no redundant CFG computation to exploit. It runs a clean deterministic 1st-order euler base (the "ddim look" + clean audio — no AB2/Heun/ancestral) and adds **video-only latent normalization**: each step it tracks the video latent's robust spread, anchors a reference once x0 is meaningful (`normalize_start_sigma`), and gently compresses any later step whose spread inflates past it (`normalize_strength`), countering the overbaking / oversaturation / colour-drift that distilled latents fall into. Benefit-only (never amplifies, no-op when the latent is healthy), ~zero overhead (a few masked reductions), and audio-safe by construction — the audio stream rides plain euler and the normalization never touches it (whole-latent for single-stream LTXV). Velocity bias + reactive rescue are available too, video-only. Node `FunPackNormalizingSampler` + Studio sampler panel.
